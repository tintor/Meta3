package tintor.devel.sokoban.cell;

import java.util.Collections;
import java.util.List;

import tintor.devel.deadlockgen.Cell;
import tintor.devel.deadlockgen.CellSearch;
import tintor.devel.deadlockgen.Dir;
import tintor.devel.deadlockgen.Key;
import tintor.devel.search.Articulations;
import tintor.devel.search.Search;
import tintor.devel.sokoban.Util;
...
public class Optimizer {
	...
	private static void fixIDs(final Key level) {
		...
		tintor.devel.deadlockgen.count = tintor.devel.deadlockgen.goals = 0;
		Collections.sort(list, tintor.devel.deadlockgen.ComparatorXY);
		for (final Cell c : list) {
			c.id = tintor.devel.deadlockgen.count;
			tintor.devel.deadlockgen.count += 1;
			if (c.goal) tintor.devel.deadlockgen.goals += 1;
		}
	}
