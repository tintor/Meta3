...
import tintor.devel.deadlockgen.Code;
import tintor.devel.deadlockgen.Dir;
import tintor.devel.deadlockgen.Key;
import tintor.devel.sokoban.common.Grid;
import tintor.devel.sokoban.common.StringGrid;
...
public class Loader {
		...
		try {
			...
			while (level.read())
				if (level.name.equalsIgnoreCase(levelname)
						|| level.name.equalsIgnoreCase("Level 1" + levelname)
	...
	public static Key load(final Grid grid) {
		...
		final List<Cell> boxes = new ArrayList<Cell>((width - 2) * (height - 2));
		Cell agent = null;
