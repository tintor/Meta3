package tintor.devel.sokoban.gui;
...
import tintor.devel.sokoban.Deadlock;
import tintor.devel.sokoban.Solver;
import tintor.devel.sokoban.cell.Cell;
import tintor.devel.sokoban.common.Dir;
...
public class Main implements PaintListener, KeyListener, MouseMoveListener, MouseTrackListener, MouseListener,
	...
	private final XMenu Sokoban = new XMenu("Sokoban", new Action("Undo", "Subtract") {
		...
		public void onSelect() {
			final Thread a = new Thread() {
				@Override
				public void run() {
					final int d = model.key.distance;
					System.gc();
	...
	private final XMenu View = new XMenu("View", "Problem Set", null, check("show.Agent|Alt+A", true), check(
			"show.Boxes|Alt+B", true), check("show.IDs|Alt+I", false),
			check("show.Dead Cells|Alt+D", true), check("show.Goals|Alt+G", true), check(
					"show.Matching Goals|Alt+M", false), check("show.Push Distances|Alt+P", false),
			check("show.Articulations|Alt+R", false), check("show.Unreachable|Alt+U", true), check(
					"show.Tunnel Lengths|Alt+T", false));

	private final XMenu Settings = new XMenu("Settings", check("Remove Deadends", null), check("Compress Tunnels",
			null), null, check("Separate Simple Goal Rooms", null), null, new XMenu("Heuristic",
			"? Closest Goals", "? Matching Goals"), new XMenu("Deadlock testing", check("Fast Block",
			Deadlock.fastBlock), check("Frozen Boxes", Deadlock.frozenBoxes)), check("Macro Expansion",
			null));
	...
	public Main() {
		...
		properties.addListener("show", new PropertyListener() {
			@Override
			public void propertyChange(final String key, final Object oldValue, final Object newValue) {
				canvas.redraw();
			}
		});

		// init console
		console.setFont(new Font(null, properties.instance(Main.class, "console.font.name", "Courier New"),
				properties.instance(Main.class, "console.font.size", 9), SWT.NORMAL));
	...
	public void paintControl(final PaintEvent e) {
		...
		for (final Cell c : model.cachedCells) {
			...
			if (c.distance(Dir.East) > 1) {
				...
				e.gc.fillRectangle(cellSize, tunnelOff, (c.distance(Dir.East) - 1) * cellSize,
						tunnelSize);
			}
			if (c.distance(Dir.South) > 1) {
				e.gc.setBackground(c.dead() || c.south().dead() ? gray60 : gray75);
				e.gc.fillRectangle(tunnelOff, cellSize, tunnelSize, (c.distance(Dir.South) - 1)
						* cellSize);
			}
		...
		if (cursor != null) {
			...
			if (properties.getBool("show.tunnel_lengths")) {
				...
				for (final Dir dir : Dir.values())
					SWT2D.drawText(e.gc, (1 + dir.dx) * cellSize / 2, (1 + dir.dy) * cellSize / 2,
							Align.Center, Align.Center, cursor.distance(dir));
		...
		if (properties.getBool("show.push_distances") && cursor != null) {
			...
			for (final Cell c : model.cachedCells)
				if (c != cursor && cursor.pushes(c) != null)
					SWT2D.drawText(e.gc, c.x * cellSize + cellSize / 2, c.y * cellSize + cellSize
							/ 2, Align.Center, Align.Center, cursor.pushes(c));
		}
	}
