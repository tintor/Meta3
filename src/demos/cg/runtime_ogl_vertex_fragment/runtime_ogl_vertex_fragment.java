...
/**
 * Basic example of the use of the Cg runtime in a simple OpenGL program. Ported to Java from NVidia's original C source
 * by Christopher Kline, 06 June 2003. Original NVidia copyright is preserved in the source code.
 */
public class runtime_ogl_vertex_fragment implements GLEventListener {
	...
	public void display(final GLAutoDrawable drawable) {
		...
		CgGL.cgGLSetStateMatrixParameter(CgGL.cgGetNamedParameter(vertexProgram, "ModelViewIT"),
				CgGL.CG_GL_MODELVIEW_MATRIX, CgGL.CG_GL_MATRIX_INVERSE_TRANSPOSE);
		...
		CgGL.cgGLSetParameter3fv(CgGL.cgGetNamedParameter(fragmentProgram, "lightColor"), lightColor, 0);
	...
	void LoadCgPrograms() {
		...
		try {
			vertexProgram = CgGL.cgCreateProgramFromStream(context, CgGL.CG_SOURCE, getClass()
					.getClassLoader().getResourceAsStream(
							"demos/cg/runtime_ogl_vertex_fragment/demo_vert.cg"),
		...
		try {
			fragmentProgram = CgGL.cgCreateProgramFromStream(context, CgGL.CG_SOURCE, getClass()
					.getClassLoader().getResourceAsStream(
							"demos/cg/runtime_ogl_vertex_fragment/demo_frag.cg"),
	...
	void LoadTextures(final GL gl) {
		...
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGBA, RES, RES, 0, GL.GL_RGBA, GL.GL_FLOAT, FloatBuffer
				.wrap(data));

		// Tell Cg which texture handle should be associated with the sampler2D
		// parameter to the fragment shader.
		CgGL.cgGLSetTextureParameter(CgGL.cgGetNamedParameter(fragmentProgram, "diffuseMap"), handle[0]);
	}

	private int VERTEX(final int u, final int v, final int nu) {
		return u + v * nu;
	}
	...
	void ParametricEval(final float u, final float v, final int offsetP, final FloatBuffer p, final int offsetN,
			final FloatBuffer N) {
